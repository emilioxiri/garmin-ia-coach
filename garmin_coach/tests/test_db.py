...
def test_purge_keeps_old_activities():
    """Activities are preserved indefinitely so historical PRs survive."""
    import garmin_coach.db as db

    db_inst = make_db()
    cutoff = date.today() - timedelta(days=30)
    old_date = (cutoff - timedelta(days=400)).isoformat()
    recent_date = (cutoff + timedelta(days=1)).isoformat()
    db_inst.table("activities").insert(
        {"activityId": "old", "startTimeLocal": f"{old_date} 10:00:00"}
    )
    db_inst.table("activities").insert(
        {"activityId": "recent", "startTimeLocal": f"{recent_date} 10:00:00"}
    )
    with patch_db(db_inst):
        db.purge_old_data(days=30)
        remaining = db_inst.table("activities").all()
    assert len(remaining) == 2
    ids = {a["activityId"] for a in remaining}
...
def test_purge_returns_removed_counts():
    ...
    old_date = (cutoff - timedelta(days=1)).isoformat()
    db_inst.table("activities").insert(
        {"activityId": "a1", "startTimeLocal": f"{old_date} 10:00:00"}
    )
    db_inst.table("activities").insert(
        {"activityId": "a2", "startTimeLocal": f"{old_date} 11:00:00"}
    )
    db_inst.table("sleep").insert({"date": old_date})
    with patch_db(db_inst):
        removed = db.purge_old_data(days=30)
    # Activities are preserved indefinitely.
    assert removed["activities"] == 0
    ...
    for t in ("training_status", "training_readiness", "respiration", "spo2", "stress"):
        assert removed[t] == 0
    assert len(db_inst.table("activities").all()) == 2
...
def test_get_context_for_ai_filters_old_activities():
    ...
    db_inst.table("activities").insert(
        {"activityId": "old", "startTimeLocal": f"{old} 08:00:00"}
    )
    db_inst.table("activities").insert(
        {"activityId": "new", "startTimeLocal": f"{recent} 08:00:00"}
    )
    with patch_db(db_inst):
        ctx = db.get_context_for_ai(days=7)
...
def test_get_context_for_ai_caps_at_20_activities():
    ...
    for i in range(25):
        db_inst.table("activities").insert(
            {"activityId": str(i), "startTimeLocal": f"{today} {i:02d}:00:00"}
        )
    with patch_db(db_inst):
        ctx = db.get_context_for_ai(days=7)
...
def test_get_context_for_ai_activities_sorted_descending():
    ...
    for i in range(3):
        ...
        db_inst.table("activities").insert(
            {"activityId": str(i), "startTimeLocal": f"{d} 08:00:00"}
        )
    with patch_db(db_inst):
        ctx = db.get_context_for_ai(days=7)
...
def test_get_context_for_ai_includes_all_memory():
    ...
    db_inst.table("memory").insert(
        {"note": "rodilla derecha", "created_at": "2020-01-01"}
    )
    db_inst.table("memory").insert(
        {"note": "lesión antigua", "created_at": "2019-01-01"}
    )
    with patch_db(db_inst):
        ctx = db.get_context_for_ai(days=7)
...
def test_get_last_sync_returns_latest():
    ...
    db_inst = make_db()
    db_inst.table("sync_log").insert(
        {"synced_at": "2024-01-10T08:00:00", "summary": {}}
    )
    db_inst.table("sync_log").insert(
        {"synced_at": "2024-01-20T08:00:00", "summary": {}}
    )
    db_inst.table("sync_log").insert(
        {"synced_at": "2024-01-15T08:00:00", "summary": {}}
    )
    with patch_db(db_inst):
